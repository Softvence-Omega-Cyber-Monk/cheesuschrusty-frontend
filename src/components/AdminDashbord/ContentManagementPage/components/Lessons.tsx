  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-gray-100">Lessons</h2>
          <p className="text-slate-500 dark:text-gray-400 mt-1">Manage structured Italian learning lessons</p>
        </div>
        <button
          onClick={() => setIsCreating(true)}
          className="bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-sm hover:shadow-md"
        >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden border border-slate-200 dark:border-gray-700 transition-colors">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-slate-500 dark:text-gray-300">
            ...
            <tbody>
              {lessons.map((lesson) => (
                <tr key={lesson.id} className="bg-white dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700 hover:bg-slate-50/70 dark:hover:bg-gray-700 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-gray-100">
                    <div>{lesson.title}</div>
                    <div className="text-xs text-slate-400 dark:text-gray-400 font-normal">{lesson.description}</div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-3">
                      <button className="text-slate-400 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400"><EyeIcon /></button>
                      <button className="text-slate-400 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400"><PencilIcon /></button>
                      <button className="text-slate-400 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400"><BlockIcon /></button>
                    </div>
                  </td>
                </tr>
              ))}
