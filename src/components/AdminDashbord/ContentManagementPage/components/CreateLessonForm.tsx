    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 rounded-xl shadow-sm">
            <div className="flex justify-between items-center border-b border-slate-200 pb-6">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900">Create New Lesson</h2>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Enter lesson title" className="mt-1 block w-full input" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Difficulty Level</label>
                        <select value={difficulty} onChange={e => setDifficulty(e.target.value as Difficulty)} className="mt-1 block w-full input">
                            {Object.values(Difficulty).map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Category</label>
                        <select value={category} onChange={e => setCategory(e.target.value as Category)} className="mt-1 block w-full input">
                             {Object.values(Category).map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Estimated Duration</label>
                        <select value={estimatedDuration} onChange={e => setEstimatedDuration(e.target.value)} className="mt-1 block w-full input">
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-slate-700">Lesson Description</label>
                        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe what student will learn in this lesson" className="mt-1 block w-full input" required />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-6 border-t border-slate-200 pt-8">
                 ...
                 <div className="space-y-6">
                    {sections.map(section => (
                        <div key={section.id} className="border border-slate-200 p-4 rounded-lg bg-slate-50/50 space-y-4">
                            <div className="flex justify-between items-center">
                                <h4 className="font-semibold text-blue-600 flex items-center gap-2">{getSectionIcon(section.type)} {section.type}</h4>
                                <button type="button" onClick={() => removeSection(section.id)} className="text-slate-400 hover:text-red-500"><TrashIcon /></button>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Section Title</label>
                                <input type="text" value={section.title} onChange={e => handleSectionChange(section.id, 'title', e.target.value)} placeholder="Enter section title..." className="mt-1 block w-full input bg-white" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Content</label>
                                <textarea value={section.content} onChange={e => handleSectionChange(section.id, 'content', e.target.value)} placeholder="Enter Italian words with translations..." className="mt-1 block w-full input bg-white min-h-[100px]" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="file-upload-btn"> <UploadIcon /> Upload Audio </div>
                                <div className="file-upload-btn"> <UploadIcon /> Upload Image </div>
                            </div>
                        </div>
                    ))}
                 </div>
            ...
            <div className="space-y-4 border-t border-slate-200 pt-8">
                <h3 className="text-lg font-semibold text-slate-900">Lesson Access</h3>
                <div className="flex items-center gap-8">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="access" value={LessonAccess.Free} checked={access === LessonAccess.Free} onChange={() => setAccess(LessonAccess.Free)} className="h-4 w-4 text-blue-600 border-slate-300 focus:ring-blue-500" />
                        Free for all users
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="access" value={LessonAccess.Premium} checked={access === LessonAccess.Premium} onChange={() => setAccess(LessonAccess.Premium)} className="h-4 w-4 text-blue-600 border-slate-300 focus:ring-blue-500"/>
                        Premium users only
                    </label>
                </div>
            </div>
