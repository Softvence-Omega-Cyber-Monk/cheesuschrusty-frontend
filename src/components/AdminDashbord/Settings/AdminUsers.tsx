...
const roleColors: Record<Role, string> = {
  [Role.SuperAdmin]: 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100',
  [Role.ContentManager]: 'bg-blue-100 text-blue-800 dark:bg-blue-700 dark:text-blue-100',
  [Role.SupportManager]: 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100',
};

const UserCard: React.FC<{ user: AdminUser }> = ({ user }) => (
  <div className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 transition-colors bg-white dark:bg-gray-800 border-b border-slate-200 dark:border-gray-700">
    <div className="mb-4 md:mb-0">
      <div className="flex items-center space-x-3">
        <h3 className="text-md font-semibold text-slate-900 dark:text-gray-100">{user.name}</h3>
    ...
    <div className="flex items-center space-x-4 w-full md:w-auto">
      <p className="text-sm text-slate-500 dark:text-gray-400">Last active: {user.lastActive}</p>
      <button className="text-sm font-medium cursor-pointer text-slate-600 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-slate-50 dark:hover:bg-gray-600 border border-slate-300 dark:border-gray-600 rounded-md px-3 py-1.5 transition">
        Edit
      </button>
      <button className="text-sm font-medium cursor-pointer text-slate-600 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-slate-50 dark:hover:bg-gray-600 border border-slate-300 dark:border-gray-600 rounded-md px-3 py-1.5 transition">
        Remove
